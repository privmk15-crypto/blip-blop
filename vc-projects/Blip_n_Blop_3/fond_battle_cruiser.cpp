...
void FondBattlecruiser::update()
{
	...
	//if (attack_etape==0)
	{
		x += 1;
		x_adversaire -= 1;
		pic = g_game_state.picture_banks().niveau()[anime(anim_cruiser_droite, 4, 12)];
		...
		if (dif_y != 0 && (dif_x / dif_y < 2)/*&&(dif_x<600)*/ && (dif_x > 0) && (dif_x != 0)) {
			...
			//	for (int i=-2;i<3;i++)
			//	{
			pic = g_game_state.picture_banks().niveau()[26 + attack_etape];
			...
			//{
			//	pic=g_game_state.picture_banks().niveau()[13];
			//}
	...
	/*else
	{
		x-=1;
		x_adversaire+=1;
		pic = g_game_state.picture_banks().niveau()[anime(anim_cruiser_gauche, 4, 8)];
	}*/
}
