...
{
	pic = g_game_state.picture_banks().niveau()[142];
}
...
void FondPokeLevel3::update()
{
	...
	if (etape < 57) {
		pic = g_game_state.picture_banks().niveau()[PBK_3 + anim_clignotement[etape]];
	}
}

void FondPokeLevel3::affiche(RenderQueue& rq)
{
	Sprite::affiche(rq);
